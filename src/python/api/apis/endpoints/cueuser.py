...
from utils.cueuser import (create_cueuser, get_cueuser, update_cueuser,
                           delete_cueuser, list_cueusers,
                           CueuserNotFoundError, get_cueuser_by_lookup,
                           get_cueuser_role, get_cognito_client)
...
async def create_cueuser_endpoint(cueuser: CueuserCreate, cognito_client = Depends(get_cognito_client)):
    try:
...
async def get_cueuser_endpoint(cueuser_id: UUID, ngroup_id: UUID = Query(... , description="ngroup ID to filter")):
    try:
...
async def update_cueuser_endpoint(cueuser_id: UUID, cueuser_update: CueuserUpdate):
    try:
...
async def delete_cueuser_endpoint(cueuser_id: UUID, ngroup_id: UUID = Query(...), cognito_client = Depends(get_cognito_client)):
    try:
