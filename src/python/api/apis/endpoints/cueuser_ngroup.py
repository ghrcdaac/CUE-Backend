...
async def associate_cueuser_with_ngroup_endpoint(data: CueuserNgroupCreate, user: CueuserAuth = Depends(get_current_user)):
    try:
...
async def dissociate_cueuser_from_ngroup_endpoint(data: CueuserNgroupCreate, user: CueuserAuth = Depends(get_current_user)):
    try:
...
async def list_ngroups_for_cueuser_endpoint(cueuser_id: UUID, user: CueuserAuth = Depends(get_current_user)):
    try:
...
async def list_cueusers_in_ngroup_endpoint(ngroup_id: UUID, user: CueuserAuth = Depends(get_current_user)):
    try:
