...
async def create_ngroup_endpoint(ngroup: NgroupCreate,  request: Request,  user: CueuserReturn = Depends(get_current_user)):
...
async def get_ngroup_id_endpoint(
    ...
    user: CueuserReturn = Depends(get_current_user)
):
    try:
...
async def get_ngroup_endpoint(ngroup_id: UUID, user: CueuserReturn = Depends(get_current_user)):
    try:
...
async def update_ngroup_endpoint(ngroup_id: UUID, ngroup_update: NgroupUpdate, user: CueuserReturn = Depends(get_current_user)):
    try:
...
async def delete_ngroup_endpoint(ngroup_id: UUID, user: CueuserReturn = Depends(get_current_user)):
    try:
...
async def list_ngroups_endpoint(user: CueuserReturn = Depends(get_current_user)):
