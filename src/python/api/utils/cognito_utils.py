...
class CognitoAuth:
    ...
    def __init__(self):
        ...
        self.client = boto3.client("cognito-idp", region_name=self.region) # Create cognito client
        self._keys = None  # Cache keys
    ...
    async def get_current_user(self, request: Request) -> Optional[dict]:
        ...
        if not auth_header:
            return None  # Or raise HTTPException if you want to require it

        try:
            bearer_token = auth_header.split(" ")[1]
            user = self.verify_token(bearer_token)  # Verify the token
            return user
        except Exception:  # Catch any verification errors
    ...
    def refresh_tokens(self, refresh_token: str) -> dict:
        ...
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,  # More general error
                detail=str(e),
            ) from e
    ...
    def verify_email(self, access_token: str) -> dict:
        ...
        try:
            ...
            return {'email_verified': False}  # Attribute not found
        except self.client.exceptions.NotAuthorizedException as e:
            raise HTTPException(
