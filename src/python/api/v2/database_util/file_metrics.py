...
def _build_metrics_query_parts(
    ngroup_id: UUID, filters: Dict[str, Any]
) -> Tuple[str, List[Any]]:
...
async def get_daily_volume(conn: Connection, ngroup_id: UUID, filters: Dict[str, Any]) -> List[Dict[str, Any]]:
...
async def get_daily_count(conn: Connection, ngroup_id: UUID, filters: Dict[str, Any]) -> List[Dict[str, Any]]:
    query_suffix, params = _build_metrics_query_parts(ngroup_id, filters)
    query = f"SELECT DATE_TRUNC('day', fs.upload_time) AS day, COUNT(f.id) AS value {query_suffix} GROUP BY day ORDER BY day;"
    return await conn.fetch(query, *params)

async def get_overall_volume(conn: Connection, ngroup_id: UUID, filters: Dict[str, Any]) -> int:
    query_suffix, params = _build_metrics_query_parts(ngroup_id, filters)
    query = f"SELECT SUM(f.size_bytes) AS value {query_suffix};"
    return await conn.fetchval(query, *params) or 0

async def get_overall_count(conn: Connection, ngroup_id: UUID, filters: Dict[str, Any]) -> int:
    query_suffix, params = _build_metrics_query_parts(ngroup_id, filters)
    query = f"SELECT COUNT(f.id) AS value {query_suffix};"
    return await conn.fetchval(query, *params) or 0

async def get_status_counts(conn: Connection, ngroup_id: UUID, filters: Dict[str, Any]) -> List[Dict[str, Any]]:
    query_suffix, params = _build_metrics_query_parts(ngroup_id, filters)
    query = f"SELECT fs.status, COUNT(f.id) AS count {query_suffix} GROUP BY fs.status;"
    return await conn.fetch(query, *params)

# ... (Cost metrics functions remain the same) ...
async def get_cost_summary_data(conn: Connection, ngroup_id: UUID, filters: Dict[str, Any]) -> Dict[str, Any]:
    query_suffix, params = _build_metrics_query_parts(ngroup_id, filters)
...
async def count_cost_by_collection(conn: Connection, ngroup_id: UUID, filters: Dict[str, Any]) -> int:
    query_suffix, params = _build_metrics_query_parts(ngroup_id, filters)
    count = await conn.fetchval(f"SELECT COUNT(DISTINCT c.id) {query_suffix}", *params)
    return count or 0

async def get_cost_by_file(conn: Connection, ngroup_id: UUID, filters: Dict[str, Any], limit: int, offset: int) -> List[Dict[str, Any]]:
    query_suffix, params = _build_metrics_query_parts(ngroup_id, filters)
    join_clause = " LEFT JOIN cost_metric cm ON f.id = cm.file_id "
...
async def count_cost_by_file(conn: Connection, ngroup_id: UUID, filters: Dict[str, Any]) -> int:
    query_suffix, params = _build_metrics_query_parts(ngroup_id, filters)
