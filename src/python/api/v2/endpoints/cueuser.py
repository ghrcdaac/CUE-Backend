...
# The prefix has been changed from "/users" to "/cueusers".
# ==============================================================================
...
from ..utils import cueuser as cueuser_utils
from ..utils.keycloak_admin import get_keycloak_admin_client, KeycloakAdminClient
from ..type_util.cueuser import UserResponse, UserCreateRequest, UserUpdateRequest, UserFindResponse
...
async def create_user_endpoint(
    ...
    try:
        new_user = await cueuser_utils.create_new_user(
            email=request.email, name=request.name, username=request.username,
...
async def find_user_endpoint(
    ...
    username: Optional[str] = Query(None, description="Username to search for (case-insensitive, partial match)."),
    ...
    try:
        users = await cueuser_utils.find_users_by_criteria(email, username, name, edpub_id)
...
@router.get("/by_username/{username}", response_model=UserResponse, dependencies=[Depends(require_privilege("view_provider"))])
async def get_user_by_username_endpoint(username: str):
    """Retrieves a specific user's profile by their username."""
    try:
        user_profile = await cueuser_utils.get_user_profile_by_username(username)
        return UserResponse(**user_profile)
    except cueuser_utils.UserNotFoundError:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="User not found.")
