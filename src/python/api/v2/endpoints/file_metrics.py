...
async def get_metrics_summary_endpoint(
    ...
    try:
        summary_data = await metrics_utils.get_metrics_summary(ngroup_id, filters)
...
async def list_files_by_status_endpoint(
    ...
    ngroup_id = UUID(user.active_ngroup_id)
    try:
        items, total = await metrics_utils.list_files_by_status(ngroup_id, status, filters, page, page_size)
...
async def get_cost_by_collection_endpoint(
    ...
    ngroup_id = UUID(user.active_ngroup_id)
    try:
        items, total = await metrics_utils.get_cost_by_collection(ngroup_id, filters, page, page_size)
