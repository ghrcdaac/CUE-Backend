# ==============================================================================
# File: src/python/api/v2/endpoints/ngroup.py (Final)
# Purpose: Provides the v2 REST API endpoints for ngroup management.
# Change: Added more comprehensive error handling for a robust API.
# ==============================================================================
...
async def create_ngroup_endpoint(ngroup: NgroupCreate):
    ...
    try:
        new_ngroup = await ngroup_utils.create_ngroup(ngroup)
...
async def list_ngroups_for_application_form():
    """
    ...
    """
    try:
        return await ngroup_utils.list_ngroups_for_form()
    except Exception:
...
async def list_ngroups_endpoint():
    ...
    try:
        return await ngroup_utils.list_ngroups()
    except Exception:
...
async def get_ngroup_endpoint(ngroup_id: UUID):
    ...
    try:
        ngroup = await ngroup_utils.get_ngroup(ngroup_id)
...
async def update_ngroup_endpoint(ngroup_id: UUID, ngroup_update: NgroupUpdate):
    ...
    try:
        updated_ngroup = await ngroup_utils.update_ngroup(ngroup_id, ngroup_update)
...
async def delete_ngroup_endpoint(ngroup_id: UUID):
    ...
    try:
        await ngroup_utils.delete_ngroup(ngroup_id)
    except ngroup_utils.NgroupNotFoundError as e:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(e))
    except Exception:
