...
async def create_role_endpoint(request: Request, role: RoleCreate):
    """Creates a new role. Requires 'admin' privilege."""
    try:
        new_role = await role_utils.create_role(request, role)
...
async def list_roles_endpoint(request: Request):
    ...
    try:
        return await role_utils.list_roles(request)
    except Exception:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="An unexpected error occurred.")
...
async def lookup_role_endpoint(
    ...
    try:
        role = await role_utils.get_role_by_lookup(request, short_name, long_name)
...
async def get_role_endpoint(request: Request, role_id: UUID):
    """Retrieves a single role by its ID. Requires 'admin' privilege."""
    try:
        role = await role_utils.get_role(request, role_id)
        return RoleResponse.model_validate(role)
    except role_utils.RoleNotFoundError as e:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(e))

@router.patch("/{role_id}", response_model=RoleResponse, dependencies=[Depends(require_privilege("role:update"))])
async def update_role_endpoint(request: Request, role_id: UUID, role_update: RoleUpdate):
    """Updates an existing role. Requires 'admin' privilege."""
    try:
        updated_role = await role_utils.update_role(request, role_id, role_update)
...
async def delete_role_endpoint(request: Request, role_id: UUID):
    """Deletes a role. Requires 'admin' privilege."""
    try:
        await role_utils.delete_role(request, role_id)
    except role_utils.RoleNotFoundError as e:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(e))
