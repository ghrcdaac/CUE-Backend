...
# Change: Standardized on 'cueusername' instead of 'username' for consistency.
# ==============================================================================
...
def _parse_user_data(user_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if isinstance(parsed_data.get("roles"), str):
        parsed_data["roles"] = json.loads(parsed_data["roles"])
    if isinstance(parsed_data.get("ngroups"), str):
        parsed_data["ngroups"] = json.loads(parsed_data["ngroups"])
    if isinstance(parsed_data.get("privileges"), str):
        parsed_data["privileges"] = json.loads(parsed_data["privileges"])
...
async def get_user_profile(user_id: UUID) -> Dict[str, Any]:
    """Fetches and parses a user's complete profile."""
    async with get_db_connection() as conn:
