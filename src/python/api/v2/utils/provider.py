# ==============================================================================
# File: src/python/api/v2/utils/provider.py (Final)
# Purpose: Contains the business logic for provider management.
# ==============================================================================
...
async def create_provider(provider: ProviderCreate) -> Dict[str, Any]:
    ...
    try:
        await cueuser_utils.get_user_profile(provider.point_of_contact)
...
async def get_provider(provider_id: UUID) -> Dict[str, Any]:
...
async def list_providers(ngroup_id: UUID) -> List[Dict[str, Any]]:
...
async def list_providers_for_form(ngroup_id: UUID) -> List[Dict[str, Any]]:
...
async def update_provider(provider_id: UUID, provider_update: ProviderUpdate) -> Dict[str, Any]:
    ...
    if "point_of_contact" in update_data:
        try:
            await cueuser_utils.get_user_profile(update_data["point_of_contact"])
        except cueuser_utils.UserNotFoundError as e:
