...
def handler(event, context):
    ...
    try:
        if query_state == "SUCCEEDED":
            results_json = get_query_results_as_json(query_exec_id)
            store_result_in_s3(results_json, result_key)
            logger.info("athena.query.succeeded", query_id=query_exec_id)

        elif query_state == "FAILED":
            error_reason = get_query_error_reason(query_exec_id)
            error_json = json.dumps({"detail": "Failed", "message": error_reason})
            store_result_in_s3(error_json, result_key)
