...
async def create_user_application_in_db(conn: Connection, params: Tuple) -> List:
    ...
    insert_query = """
        INSERT INTO user_application (email, name, applied, username, status, ngroup_id, justification)
        VALUES ($1, $2, $3, $4, $5, $6, $7)
        RETURNING id, email, name, applied, username, status, ngroup_id, justification
    """
    try:
        ...
        raise ValueError("Invalid ngroup_id provided.")
    except DataError as e:
        logger.error(f"Failed to create user_application due to invalid data: {e}", exc_info=True)
        raise ValueError("Invalid data provided for creating a user_application.")
...
async def get_user_application_from_db(conn: Connection, params: Tuple) -> List:
    ...
    select_query = """
        SELECT id, email, name, applied, username, status, ngroup_id, justification
...
async def update_user_application_in_db(conn: Connection, params: Tuple) -> List:
    ...
    try:
        ...
        raise ValueError("Invalid data provided for updating a user_application.")
    except Exception as e:
        logger.error(f"An unexpected error occurred while updating a user_application: {e}", exc_info=True)
        raise
