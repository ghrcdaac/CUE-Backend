...
data "template_file" "cue_api_lambda_assume_role" {
    template = file("./iam/cue_api_lambda_assume_role.json")
    vars = {
        region = var.region
        account_id = var.account_id
    }
...
data "template_file" "cue_api_lambda_policy" {
    template = file("./iam/cue_api_lambda_policy.json")
    vars = {
        region = var.region
        account_id = var.account_id
    }
