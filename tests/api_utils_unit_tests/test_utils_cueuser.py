...
from app.v2.utils.cueuser import create_new_user, get_user_profile, list_users, get_user_profile_by_username, find_users_by_criteria, get_users_by_role, delete_user_fully, update_user_details, update_user_role, UserNotFoundError
...
async def test_create_new_user(make_request, test_ngroup_id, test_provider):
    ...
    profile = await create_new_user(req, user_id, email, name, cueusername, role_id, edpub_id, [test_ngroup_id], [test_provider["id"]])
    assert profile["id"] == user_id
...
async def test_list_users(make_request, test_admin_user, make_cueuser):
    ...
    profile1 = await create_new_user(req, user1["user_id"], user1["email"], user1["name"], user1["cueusername"], user1["role_id"], user1["edpub_id"], user1["ngroup_ids"], user1["provider_ids"])
    profile2 = await create_new_user(req, user2["user_id"], user2["email"], user2["name"], user2["cueusername"], user2["role_id"], user2["edpub_id"], user2["ngroup_ids"], user2["provider_ids"])
...
@pytest.mark.asyncio
async def test_get_user_profile_by_username_username_does_not_exist(make_request, test_admin_user):
    req = make_request()
    with pytest.raises(UserNotFoundError):
...
async def test_find_users_by_criteria(make_request, test_admin_user, make_cueuser):
    ...
    # edpub_id
    user = make_cueuser(uuid.uuid4(), "test_user1@test.com", "test_user1", "test_username1", role_id="0e686dba-e5b2-4302-aea0-e9ed0caff7d3", edpub_id=str(uuid.uuid4()))
...
async def test_get_user_by_role(make_request, make_cueuser):
    ...
    req = make_request()
    admin_user = make_cueuser(uuid.uuid4(), "test_admin@test.com", "test_adminuser", "test_admin", role_id="c924d0d3-55af-49f3-bec1-d7fd4ed475e2", edpub_id=None)
    await create_new_user(req, admin_user["user_id"], admin_user["email"], admin_user["name"], admin_user["cueusername"], admin_user["role_id"], admin_user["edpub_id"], admin_user["ngroup_ids"], admin_user["provider_ids"]) 

    security_user = make_cueuser(uuid.uuid4(), "test_security@test.com", "test_sec", "test_secuser", role_id="39677929-ba9b-426d-8c18-f607d669fcce", edpub_id=None)
    await create_new_user(req, security_user["user_id"], security_user["email"], security_user["name"], security_user["cueusername"], security_user["role_id"], security_user["edpub_id"], security_user["ngroup_ids"], security_user["provider_ids"]) 

    manager_user = make_cueuser(uuid.uuid4(), "test_manager@test.com", "test_manager", "test_manageruser", role_id="ef872fe7-92b9-45ec-ac19-80f4c478fd36", edpub_id=None)
    await create_new_user(req, manager_user["user_id"], manager_user["email"], manager_user["name"], manager_user["cueusername"], manager_user["role_id"], manager_user["edpub_id"], manager_user["ngroup_ids"], manager_user["provider_ids"]) 

    staff_user1 = make_cueuser(uuid.uuid4(), "test_staff1@test.com", "test_staff1", "test_staffuser1", role_id="a8b3757b-dcf9-4943-8f64-5adaf17a17fe", edpub_id=None)
    await create_new_user(req, staff_user1["user_id"], staff_user1["email"], staff_user1["name"], staff_user1["cueusername"], staff_user1["role_id"], staff_user1["edpub_id"], staff_user1["ngroup_ids"], staff_user1["provider_ids"]) 

    staff_user2 = make_cueuser(uuid.uuid4(), "test_staff2@test.com", "test_staff2", "test_staffuser2", role_id="a8b3757b-dcf9-4943-8f64-5adaf17a17fe", edpub_id=None)
    await create_new_user(req, staff_user2["user_id"], staff_user2["email"], staff_user2["name"], staff_user2["cueusername"], staff_user2["role_id"], staff_user2["edpub_id"], staff_user2["ngroup_ids"], staff_user2["provider_ids"]) 

    observer_user = make_cueuser(uuid.uuid4(), "test_observer@test.com", "test_obs", "test_obsuser", role_id="a8b3757b-dcf9-4943-8f64-5adaf17a17fe", edpub_id=None)
    await create_new_user(req, observer_user["user_id"], observer_user["email"], observer_user["name"], observer_user["cueusername"], observer_user["role_id"], observer_user["edpub_id"], observer_user["ngroup_ids"], observer_user["provider_ids"]) 

    provider_user = make_cueuser(uuid.uuid4(), "test_provider@test.com", "test_provider", "test_provideruser", role_id="0e686dba-e5b2-4302-aea0-e9ed0caff7d3", edpub_id=str(uuid.uuid4()))
    await create_new_user(req, provider_user["user_id"], provider_user["email"], provider_user["name"], provider_user["cueusername"], provider_user["role_id"], provider_user["edpub_id"], provider_user["ngroup_ids"], provider_user["provider_ids"]) 
...
@pytest.mark.asyncio
async def test_update_user_role_as_daac_manager_fail(make_request, make_cueuser, test_daac_manager_user):
    req = make_request()
...
@pytest.mark.asyncio
async def test_update_user_role_as_security_fail(make_request, make_cueuser, test_security_user):
    req = make_request()
