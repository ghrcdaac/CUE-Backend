...
def test_get_my_profile_not_found(test_client, make_jwt):
    """Test get my profile - user does not exist."""
...
@pytest.mark.asyncio
async def test_list_users_endpoint(test_client, test_admin_user, test_daac_manager_user,
                                   test_daac_staff_user, test_daac_observer_user, test_provider_user,
                                   test_ngroup_id, make_jwt):
    """Test list users endpoint - success."""
    test_admin_user_jwt = make_jwt(sub=str(test_admin_user.id))
...
def test_create_user_endpoint(test_client, test_admin_user, test_ngroup_id, make_jwt):
    """Test create user endpoint - success. """
    test_admin_user_jwt = make_jwt(sub=str(test_admin_user.id))
    headers = {"Authorization": f"Bearer {test_admin_user_jwt}"}
    user_id = uuid.uuid4()
...
def test_create_user_endpoint_user_already_exists(test_client, test_admin_user, test_ngroup_id, make_jwt):
    """Test create user endpoint - duplicate user. """
    test_admin_user_jwt = make_jwt(sub=str(test_admin_user.id))
    headers = {"Authorization": f"Bearer {test_admin_user_jwt}"}
    user_id = uuid.uuid4()
    create_user_request = {"user_id": str(user_id),
...
def test_create_users_endpoint_user_no_provider_no_ngroup(test_client, test_admin_user, make_jwt):
    """Test create user endpoint - no provider/ngroup provided. """
    test_admin_user_jwt = make_jwt(sub=str(test_admin_user.id))
    headers = {"Authorization": f"Bearer {test_admin_user_jwt}"}
    user_id = uuid.uuid4()
    create_user_request = {"user_id": str(user_id),
...
def test_find_user_endpoint(test_client, test_admin_user, test_daac_manager_user, make_jwt):
    """test find user endpoint - success."""
    test_admin_user_jwt = make_jwt(sub=str(test_admin_user.id))
    headers = {"Authorization": f"Bearer {test_admin_user_jwt}"}
...
def test_find_user_endpoint_no_criteria(test_client, test_admin_user, make_jwt):
    """Test find user endpoint - no criteria provided."""
    test_admin_user_jwt = make_jwt(sub=str(test_admin_user.id))
    headers = {"Authorization": f"Bearer {test_admin_user_jwt}"}
    response = test_client.get("v2/cueusers/find", headers=headers, params={})
    assert response.status_code == 400

def test_get_users_by_role_endpoint(test_client, test_admin_user, test_security_user,
                                    test_daac_manager_user, test_daac_staff_user, test_daac_observer_user, test_provider_user, make_jwt):
    """Test get users by role endpoint - success."""
    test_admin_user_jwt = make_jwt(sub=str(test_admin_user.id))
    headers = {"Authorization": f"Bearer {test_admin_user_jwt}"}
    # admin
...
def test_get_user_by_id_endpoint(test_client, test_admin_user, test_daac_staff_user, make_jwt):
    """Test get user by id endpoint - success."""
    test_admin_user_jwt = make_jwt(sub=str(test_admin_user.id))
    headers = {"Authorization": f"Bearer {test_admin_user_jwt}"}
...
def test_get_user_by_id_endpoint_not_found(test_client, test_admin_user, make_jwt):
    """Test get user by id endpoint - user does not exist."""
    test_admin_user_jwt = make_jwt(sub=str(test_admin_user.id))
    headers = {"Authorization": f"Bearer {test_admin_user_jwt}"}
...
def test_get_user_by_username_endpoint(test_client, test_admin_user, test_daac_observer_user, make_jwt):
    """Test get user by username endpoint - success."""
    test_admin_user_jwt = make_jwt(sub=str(test_admin_user.id))
    headers = {"Authorization": f"Bearer {test_admin_user_jwt}"}
...
def test_get_user_by_username_endpoint_does_not_exist(test_client, test_admin_user, make_jwt):
    """Test get user by username endpoint - user does not exist."""
    test_admin_user_jwt = make_jwt(sub=str(test_admin_user.id))
    headers = {"Authorization": f"Bearer {test_admin_user_jwt}"}
...
def test_update_user_endpoint(test_client, test_admin_user, test_daac_manager_user, make_jwt):
    """Test update user endpoint - success."""
    test_admin_user_jwt = make_jwt(sub=str(test_admin_user.id))
    headers = {"Authorization": f"Bearer {test_admin_user_jwt}"}
...
def test_update_user_endpoint_not_found(test_client, test_admin_user, make_jwt):
    """Test update user endpoint - user does not exist."""
    test_admin_user_jwt = make_jwt(sub=str(test_admin_user.id))
    headers = {"Authorization": f"Bearer {test_admin_user_jwt}"}
    update_request = {"name":"new test daac manager", "email": "new_daac_manager@test.com", "edpub_id":str(uuid.uuid4()) }
...
def test_update_user_endpoint_no_update(test_client, test_admin_user, test_daac_manager_user, make_jwt):
    """Test update user endpoint - empty update."""
    test_admin_user_jwt = make_jwt(sub=str(test_admin_user.id))
    headers = {"Authorization": f"Bearer {test_admin_user_jwt}"}
...
def test_update_user_role_endpoint(test_client, test_admin_user, test_daac_observer_user, make_jwt):
    """Test update user role endpoint - success."""
    test_admin_user_jwt = make_jwt(sub=str(test_admin_user.id))
    headers = {"Authorization": f"Bearer {test_admin_user_jwt}"}
...
def test_update_user_role_endpoint_user_not_found(test_client, test_admin_user, make_jwt):
    test_admin_user_jwt = make_jwt(sub=str(test_admin_user.id))
    headers = {"Authorization": f"Bearer {test_admin_user_jwt}"}
    update_request = {"role_id": "a8b3757b-dcf9-4943-8f64-5adaf17a17fe"}
...
def test_update_user_role_endpoint_role_not_found(test_client, test_admin_user, test_daac_observer_user, make_jwt):
    """Test update user role endpoint - role does not exist."""
    test_admin_user_jwt = make_jwt(sub=str(test_admin_user.id))
    headers = {"Authorization": f"Bearer {test_admin_user_jwt}"}
...
def test_delete_user_endpoint(test_client, test_admin_user, test_ngroup_id, make_jwt):
    """Test delete user endpoint - success."""
    # Create new user
    test_admin_user_jwt = make_jwt(sub=str(test_admin_user.id))
    headers = {"Authorization": f"Bearer {test_admin_user_jwt}"}
    user_id = uuid.uuid4()
...
def test_delete_user_endpoint_user_not_found(test_client, test_admin_user, make_jwt):
    """Test delete user endpoint - user does not exist"""
    # Create new user
    test_admin_user_jwt = make_jwt(sub=str(test_admin_user.id))
    headers = {"Authorization": f"Bearer {test_admin_user_jwt}"}
