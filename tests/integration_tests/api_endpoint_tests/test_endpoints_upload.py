...
def test_prepare_single_upload(test_client, test_admin_user, test_ngroup_id, test_collection, make_jwt, mock_boto3_client):
...
def test_prepare_single_upload_client_error(test_client, test_admin_user, test_ngroup_id, test_collection, make_jwt, mock_boto3_client):
...
def test_prepare_single_upload_bad_collection(test_client, test_admin_user, test_ngroup_id, test_collection, make_jwt, mock_boto3_client):
    test_admin_user_jwt = make_jwt(sub=str(test_admin_user.id))
    api_key_headers = {"Authorization": f"Bearer {test_admin_user_jwt}"}
    api_key = get_api_key(test_client, api_key_headers, test_ngroup_id)
    upload_headers = {"Authorization": f"Bearer {api_key["key"]}"}
...
def test_complete_single_upload(test_client, test_admin_user, test_ngroup_id, make_jwt, mock_boto3_client):
    test_admin_user_jwt = make_jwt(sub=str(test_admin_user.id))
    api_key_headers = {"Authorization": f"Bearer {test_admin_user_jwt}"}
    api_key = get_api_key(test_client, api_key_headers, test_ngroup_id)
    upload_headers = {"Authorization": f"Bearer {api_key["key"]}"}
...
def test_multipart_start(test_client, test_admin_user, test_ngroup_id, test_collection, make_jwt, mock_boto3_client):
    test_admin_user_jwt = make_jwt(sub=str(test_admin_user.id))
    api_key_headers = {"Authorization": f"Bearer {test_admin_user_jwt}"}
    api_key = get_api_key(test_client, api_key_headers, test_ngroup_id)
    upload_headers = {"Authorization": f"Bearer {api_key["key"]}"}
...
def test_multipart_start_bad_collection(test_client, test_admin_user, test_ngroup_id, test_collection, make_jwt, mock_boto3_client):
    test_admin_user_jwt = make_jwt(sub=str(test_admin_user.id))
    api_key_headers = {"Authorization": f"Bearer {test_admin_user_jwt}"}
    api_key = get_api_key(test_client, api_key_headers, test_ngroup_id)
    upload_headers = {"Authorization": f"Bearer {api_key["key"]}"}
...
def test_multipart_start_client_error(test_client, test_admin_user, test_ngroup_id, test_collection, make_jwt, mock_boto3_client):
    s3 = mock_boto3_client("s3")
...
def test_multipart_get_part_url(test_client, test_admin_user, test_ngroup_id, make_jwt, mock_boto3_client):
    test_admin_user_jwt = make_jwt(sub=str(test_admin_user.id))
    api_key_headers = {"Authorization": f"Bearer {test_admin_user_jwt}"}
    api_key = get_api_key(test_client, api_key_headers, test_ngroup_id)
    upload_headers = {"Authorization": f"Bearer {api_key["key"]}"}
...
def test_multipart_get_part_url_client_error(test_client, test_admin_user, test_ngroup_id, make_jwt, mock_boto3_client):
    s3 = mock_boto3_client("s3")
    s3.generate_presigned_url.side_effect=ClientError({"Error":{"Message":"Forced Error", "Code":"generate_presigned_url.failed"}}, operation_name="generate_presigned_url")
...
def test_multipart_complete(test_client, test_admin_user, test_ngroup_id, test_collection, make_jwt, mock_boto3_client):
    test_admin_user_jwt = make_jwt(sub=str(test_admin_user.id))
    api_key_headers = {"Authorization": f"Bearer {test_admin_user_jwt}"}
    api_key = get_api_key(test_client, api_key_headers, test_ngroup_id)
    upload_headers = {"Authorization": f"Bearer {api_key["key"]}"}
...
def test_multipart_complete_bad_collection(test_client, test_admin_user, test_ngroup_id, test_collection, make_jwt, mock_boto3_client):
    test_admin_user_jwt = make_jwt(sub=str(test_admin_user.id))
    api_key_headers = {"Authorization": f"Bearer {test_admin_user_jwt}"}
    api_key = get_api_key(test_client, api_key_headers, test_ngroup_id)
    upload_headers = {"Authorization": f"Bearer {api_key["key"]}"}
...
def test_multipart_complete_client_error(test_client, test_admin_user, test_ngroup_id, test_collection, make_jwt, mock_boto3_client):
    s3 = mock_boto3_client("s3")
...
def test_multipart_abort(test_client, test_admin_user, test_ngroup_id, test_collection, make_jwt, mock_boto3_client):
    test_admin_user_jwt = make_jwt(sub=str(test_admin_user.id))
    api_key_headers = {"Authorization": f"Bearer {test_admin_user_jwt}"}
    api_key = get_api_key(test_client, api_key_headers, test_ngroup_id)
    upload_headers = {"Authorization": f"Bearer {api_key["key"]}"}
...
def test_multipart_abort_client_error(test_client, test_admin_user, test_ngroup_id, test_collection, make_jwt, mock_boto3_client):
    s3 = mock_boto3_client("s3")
