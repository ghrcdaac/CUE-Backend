...
from app.v2.utils.api_keys import create_api_key, list_api_keys, update_api_key, revoke_api_key,  record_api_key_usage, ApiKeyNotFoundError, ApiKeyPermissionError 
from app.v2.type_util.api_keys import ApiKeyCreateRequest, ApiKeyUpdateRequest, ApiKeyInfo
...
async def test_create_managed_user_api_key(make_request, test_ngroup_id, test_admin_user, seed_user):
    ...
    mock_api_key_create_request = ApiKeyCreateRequest(name="test_key", key_type="managed_user", target_user_id=mock_obs_user_id, expires_in_days=10, ngroup_id=uuid.UUID(test_admin_user.ngroups[0]))
    api_key = await create_api_key(req, mock_api_key_create_request, test_admin_user)
    ...
    mock_api_key_create_request = ApiKeyCreateRequest(name="test_manager_for_observer_key", key_type="managed_user", target_user_id=mock_obs_user_id, expires_in_days=10, ngroup_id=test_ngroup_id)
...
async def test_create_managed_user_api_key_manager_not_in_ngroup(make_request, test_ngroup_id, seed_ngroup, seed_user):
    ...
    mock_daac_manager = AuthUser(id=mock_dm_user_id, email="test_daac_manager_email@test.com", 
                    ...
                    roles=["daac_manager"], ngroups=[str(test_ngroup_id)],
                    active_ngroup_id=str(test_ngroup_id))
    mock_api_key_create_request = ApiKeyCreateRequest(name="test_manager_for_observer_key", key_type="managed_user", target_user_id=mock_obs_user_id, expires_in_days=10, ngroup_id=ngroup_id2)
...
async def test_create_proxy_api_key(make_request, test_admin_user):
    req = make_request()
    mock_api_key_create_request = ApiKeyCreateRequest(name="test_key", key_type="proxy", proxy_user_name="test_proxy_user", expires_in_days=10, ngroup_id=uuid.UUID(test_admin_user.ngroups[0]))
    api_key = await create_api_key(req, mock_api_key_create_request, test_admin_user)
...
async def test_list_api_key(make_request, test_admin_user):
    req = make_request()
    mock_api_key_create_request1 = ApiKeyCreateRequest(name="test_key1", key_type="personal", expires_in_days=10, ngroup_id=uuid.UUID(test_admin_user.ngroups[0]))
    mock_api_key_create_request2 = ApiKeyCreateRequest(name="test_key2", key_type="proxy", expires_in_days=10, proxy_user_name="test_proxy", ngroup_id=uuid.UUID(test_admin_user.ngroups[0]))
...
async def test_update_api_key_as_manager_for_other(make_request, seed_user, test_ngroup_id, connection_pool):
    ...
    mock_api_key_create_request = ApiKeyCreateRequest(name="test_key", key_type="managed_user", target_user_id=mock_obs_user_id, expires_in_days=10, ngroup_id=test_ngroup_id)
...
@pytest.mark.asyncio
async def test_update_api_key_as_manager_for_other_no_permission(make_request, seed_ngroup, seed_user, test_ngroup_id, connection_pool):
    req = make_request()
...
@pytest.mark.asyncio
async def test_revoke_api_key_as_manager_for_other_no_permission(make_request, seed_ngroup, seed_user, test_ngroup_id, connection_pool):
    req = make_request()
